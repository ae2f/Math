 ...
 * */
#define __ae2f_MathIntNxt(_reterr, _idx, _int, _int_vec, _out_int, _out_int_vec) \
{ \
...
#define __ae2f_MathIntIsNegative(__i, __v) \
	((__i) && (__i)->sign && (__v) && ae2f_BitVecGet( \
			(__v)[((__i)->vecbegpoint + (__i)->sz - 1) >> 3] \
 ...
 * Its type must be an existing variable, sharing a same datatype with `__in_vec`. \n
 * If you're using a function, This could be `NULL` and is encouraged to be.
 *
 * @param ae2f_bMathMem& __o_vec
 * Its type must be an existing variable, sharing a same datatype with `__out_vec` \n
 * If you're using a function, This could be `NULL` and is encouraged to be.
 * */
#define __ae2f_MathIntCast( \
		reterr, \
		count, \
		__in, \
		__in_vec, \
...
do { \
	if((reterr) && *(reterr)) break; \
	if(!(count)) { break; } \
	if(!(__in))  { \
		if((reterr)) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; \
		break;  \
	} \
	if(!(__out)) { \
		if((reterr)) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; \
	...
	{ \
		...
		for(j = 0; j < __i.sz - ae2f_static_cast(size_t, __i.sign) && j < __o.sz; j++) \
		{ \
 ...
 * If you're using a function, This could be `NULL` and is encouraged to be.
 * */
#define __ae2f_MathIntFlip(reterr, count, __in, __in_vec, __out, __out_vec, __i_vec, __o_vec) \
do { \
	if((reterr) && *(reterr)) break; \
	\
	if(!(__in)) { \
	...
	{ \
		...
		\
		sbit = __o.sign && ae2f_BitVecGet( \
				(__i_vec)[ivecbpq] \
				, (__i_vec)[__i.vecbegpoint + __i.sz - 1] & 7 \
				); \
		\
		for(j = 0; j < __o.sz; j++) \
		{ \
 ...
 * Its type must be an existing variable, sharing a same datatype with `_out_vec`. \n
 * If you're using a function, This could be `NULL` and is encouraged to be.
 *
 * */
#define __ae2f_MathIntAdd(reterr, count, _a, _a_vec, _b, _b_vec, _out, _out_vec, __av, __bv, __ov) \
do { \
	if((reterr) && *(reterr)) \
		break; \
	\
	...
	{ \
		...
		__sb2 \
			=  \
			(__a.sign && ae2f_BitVecGet( \
					(__av)[(__a.vecbegpoint + __a.sz - 1) >> 3] \
					, (__a.vecbegpoint + __a.sz - 1) & 7 \
					)) \
			| ((__b.sign && ae2f_BitVecGet( \
					(__bv)[(__b.vecbegpoint + __b.sz - 1) >> 3] \
...
\
		if( \
				_in.sign && \
				ae2f_BitVecGet( \
					(_in_vec)[(_in.vecbegpoint + _in.sz - 1) >> 3] \
 ...
 *
 * */
#define __ae2f_MathIntCmp(errret, count, a, a_vec, b, b_vec, out, _a_vec, _b_vec) \
do { \
	if((errret) && *(errret)) return; \
	if(!(out)) { \
		if(errret) *(errret) |= ae2f_errGlob_PTR_IS_NULL; \
		break; \
	} \
	if(!(a)) { \
		if(errret) *(errret) |= ae2f_errGlob_PTR_IS_NULL; \
	...
	ae2f_MathInt  \
		...
		, _b = *(b); \
	if(_a.sz - _a.sign < 1) { \
		if(errret) *(errret) |= ae2f_errGlob_WRONG_OPERATION; \
		break; \
	} \
	if(_b.sz - _b.sign < 1) { \
		if(errret) *(errret) |= ae2f_errGlob_WRONG_OPERATION; \
	...
	{ \
		...
		c[out] = 0; \
		if( \
				(_sign = (_a.sign && ae2f_BitVecGet( \
					(_a_vec)[(_a.vecbegpoint + _a.sz - 1) >> 3] \
					, ((_a.vecbegpoint + _a.sz - 1) & 7 \
					)))) \
				!=  \
				(_b.sign && ae2f_BitVecGet( \
		...
		{ \
			const size_t \
				inavp = _a.vecbegpoint + i \
				, inavpr = inavp & 7 \
				, inavps = inavp >> 3 \
				\
				, inbvp = _b.vecbegpoint + i \
				, inbvpr = inbvp & 7 \
				, inbvps = inbvp >> 3 \
				; \
			const int8_t _cmp =  \
				ae2f_static_cast(int8_t, i < _a.sz  \
				 ? ae2f_BitVecGet((_a_vec)[inavps], inavpr) \
				 : _sign \
				 ) \
				- \
				ae2f_static_cast(int8_t, i < _b.sz  \
				 ? ae2f_BitVecGet((_b_vec)[inbvps], inbvpr) \
				 : _sign \
				 ) \
				; \
			/* is abs(a) greater */ \
...
inline static void __ae2f_MathIntBitL(
		ae2f_MathMemOutErr	reterr,
		size_t			count,
		size_t			bitcount,
		const ae2f_MathInt*	in,
		ae2f_iMathMem		in_vec,
		const ae2f_MathInt*	out,
		ae2f_oMathMem		out_vec,

		/** Buffer session */
		ae2f_bMathMem		in_vec_buf,
		ae2f_bMathMem		out_vec_buf
		)
{
	if((reterr) && *(reterr)) { return; }
	if(!((bitcount) && (count))) return;

	if(!(in)) { if(reterr) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; return; }
	if(!(in_vec)) { if(reterr) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; return; }
	if(!(out)) { if(reterr) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; return; }
	if(!(out_vec)) { if(reterr) *(reterr) |= ae2f_errGlob_PTR_IS_NULL; return; }

	if(in->sz + (bitcount) < (in)->sz) {
		if(reterr) *(reterr) |= ae2f_errGlob_IMP_NOT_FOUND;
		return;
	}
 ...
 *
 * @param buf
 * Will be used as a buffer needed for a calculation.
 *
 * size as bytes: ((2 + (`b`->sz) + (`out`->sz)) / 8) + !!((2 + (`b`->sz) + (`out`->sz)) % 8)
 * */
inline static void __ae2f_MathIntMul(
		ae2f_MathMemOutErr reterr,
		size_t count,
		const ae2f_MathInt* a,
		ae2f_iMathMem a_vec,
		const ae2f_MathInt* b,
		ae2f_iMathMem b_vec,
		const ae2f_MathInt* out,
		ae2f_oMathMem out_vec,
		ae2f_bMathMem a_vec_buf,
		ae2f_bMathMem b_vec_buf,
		ae2f_bMathMem o_vec_buf
		)
{
	if((reterr) && *(reterr)) return;
